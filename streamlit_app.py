        ...
        if st.session_state.get("mode", "").startswith("2 cámaras"):
            ...
            if ready:
                ...
                for reason in st.session_state.get("ready_3d_reasons", []):
                    st.write(f"• {reason}")
            st.warning("La Versión 25 **no triangula todavía coordenadas 3D**. Los resultados siguientes continúan siendo 2D, pero cada plano se interpreta desde la cámara apropiada.")
...
st.divider()
st.caption("PhysioSentinel Gait Versión 30 · ICLM (Índice de Capacidad Locomotora) 0.1 capacidad locomotora · IBV (Índice Biomecánico dependiente de la Vista) 0.2 interpretado por plano de movimiento · valgo absoluto y pie/retropié no penalizadores en frontal · regla de déficit dominante · COM (Centro de Masa estimado 2D) · consistencia paso a paso · anexo completo · exportación")
